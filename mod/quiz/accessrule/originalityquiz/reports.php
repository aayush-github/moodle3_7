...
$url = new moodle_url('/mod/quiz/accessrule/originalityquiz/reports.php', array('id' => $cm->id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
$file = $CFG->dirroot . '/mod/quiz/accessrule/originalityquiz/report_class.php';

if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'originality_quiz_' . $mode . '_report';
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'quiz');
}

echo "<style>.mform{ display : none }</style>";
