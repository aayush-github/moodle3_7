 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class originality_quiz_overview_table extends quiz_attempts_report_table {
     ...
     */
    public function col_plagrism($attempt) {
        global $DB, $USER, $COURSE, $OUTPUT, $CFG, $PAGE;

        $questionsattempts = $DB->get_record('question_usages', array('id' => $attempt->usageid ));
        $contextid = $DB->get_record('context', array('id' => $questionsattempts->contextid ));

        $linkarray['cmid'] = $contextid->instanceid;
        // $linkarray['file'] = null;
        $linkarray['userid'] = $attempt->userid;
        ...
        if ($resp) {
            ...
            if ($req) {
                if ($req->upload_error == 1) {
                    ...
                    $output .= get_string("originality_before_turned_on", "plagiarism_originality");
                    $output .= '</div>';
            }
        }
        $output .= '<hr />';
